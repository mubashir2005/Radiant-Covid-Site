...
function App() {
  ...
  const handleMedicalResourcesChange = (query: string) => {
    firebaseApp.analytics().logEvent(`changed filters to ${query}`, {
      filter: query,
    });
  ...
  return (
    <div className={"flex justify-center flex-col pb-4"}>
      ...
      <Error
        errorMessage={
          tweets.error
            ? "We went into a bug. Try searching with searchbar"
            : "No tweets found. Please use the searchbar"
        }
